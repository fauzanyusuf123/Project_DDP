...
if "users" not in st.session_state:
    st.session_state.users = {"user1": "12345"}  # Data pengguna contoh
...
if st.session_state.logged_in:
    ...
    #Riwayat pesanan
        ...
        if st.session_state.current_user and st.session_state.order_history.get(st.session_state.current_user):
            for order in st.session_state.order_history[st.session_state.current_user]:
                ...
                for item, quantity in order["items"].items():
                    if item in menu_makanan:
                        ...
                        harga = menu_minuman[item]
                    st.write(f"{item}: {quantity} x Rp {harga:,} = Rp {harga * quantity:,}")
                st.write(f"Total: Rp {order['total']:,}")
                st.write(f"Metode Pembayaran: {order['payment_method']}")
                st.markdown("---")
            else:
                st.write("Tidak ada riwayat pesanan.")                